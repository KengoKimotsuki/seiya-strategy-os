...
import { generateMultiRoomLayout } from './simulation/generateMultiRoomLayout.ts';
...
export function dispatchMockMessages(): void {
  ...
  // Use the multi-room layout for simulation mode instead of the default single-room
  const effectiveLayout = generateMultiRoomLayout() ?? layout;
  ...
  dispatch({ type: 'layoutLoaded', layout: effectiveLayout });
  dispatch({
    type: 'settingsLoaded',
    soundEnabled: false,
