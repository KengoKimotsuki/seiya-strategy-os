...
import { CockpitApp } from './components/cockpit/CockpitApp.js';
...
function App() {
  ...
  // Browser mode: pan camera to room center when room changes
  useEffect(() => {
    if (!isBrowserRuntime) return;
    const handler = (event: MessageEvent) => {
      const msg = event.data as Record<string, unknown> | undefined;
      if (!msg || msg.type !== 'simulationRoomChanged') return;
      const cam = msg.cameraCenterPx as { x: number; y: number } | undefined;
      if (!cam) return;
      // Pan offset = negative of (room center in map-pixel space) scaled by zoom
      // This centers the room in the viewport
      const zoom = editor.zoom;
      editor.panRef.current.x = -(cam.x * zoom);
      editor.panRef.current.y = -(cam.y * zoom);
    };
  ...
  // ブラウザ単体（npm run dev）では戦略コックピット（SEIYA）を表示する。
  // VS Code 拡張ホスト時は従来のピクセルオフィス UI を維持する（元 Pixel シミュレーター無改変）。
  if (isBrowserRuntime) {
    return <CockpitApp />;
  }

  if (!layoutReady) {
    return <div className="w-full h-full flex items-center justify-center ">読み込み中...</div>;
  }
    ...
    <div ref={containerRef} className="w-full h-full relative overflow-hidden">
      ...
      {!isDebugMode ? (
        <>
          ...
          {isBrowserRuntime && (
            <ErrorBoundary label="SimulationPanel">
              <SimulationPanel />
            </ErrorBoundary>
          )}
          {isBrowserRuntime && (
            <ErrorBoundary label="DiscussionPanel">
              <DiscussionPanel />
            </ErrorBoundary>
          )}

          {/* Vignette overlay — room-aware in browser mode */}
          <div
            className="absolute inset-0 pointer-events-none"
            style={{
              background: isBrowserRuntime
                ? simulationController.getRoom().vignetteColor
                : 'var(--vignette)',
            }}
          />
...
詳しく見る
      ...
      >
        <div className="text-base text-text px-10" style={{ lineHeight: 1.4 }}>
          <p className="mb-8">Pixel Agents のオフィスがリアルタイムで反応します:</p>
          <ul className="mb-8 pl-18 list-disc m-0">
            <li className="text-sm mb-2">権限確認ダイアログが即座に表示されます</li>
            <li className="text-sm mb-2">ターン完了をその瞬間に検知します</li>
            <li className="text-sm mb-2">通知音が遅延なく再生されます</li>
          </ul>
          <p className="mb-12 text-text-muted">
            これは Claude Code Hooks(Claude セッションで何かが起きたときに Pixel Agents に通知する小さなイベントリスナー)を通じて動作します。
          </p>
