...
export class OfficeState {
  ...
  /**
   * Move all non-sub-agent characters to seats within a given tile rectangle.
   * Used by the simulation room-switch feature to relocate everyone to the
   * target room.
   */
  reassignCharactersToRoom(bounds: {
    x: number;
    y: number;
    width: number;
    height: number;
  }): void {
    // Collect seats inside the bounds
    const roomSeats: string[] = [];
    for (const [uid, seat] of this.seats) {
      if (
        seat.seatCol >= bounds.x &&
        seat.seatCol < bounds.x + bounds.width &&
        seat.seatRow >= bounds.y &&
        seat.seatRow < bounds.y + bounds.height
      ) {
        roomSeats.push(uid);
      }
    }
    ...
    // Unassign all non-sub-agent characters from their current seats
    for (const ch of this.characters.values()) {
      if (ch.isSubagent) continue;
      if (ch.seatId) {
        const old = this.seats.get(ch.seatId);
    ...
    // Reassign each character to a free seat in the room
    let seatIdx = 0;
    for (const ch of this.characters.values()) {
      if (ch.isSubagent) continue;
      ...
      for (let i = 0; i < roomSeats.length; i++) {
        const candidateUid = roomSeats[(seatIdx + i) % roomSeats.length]!;
        const seat = this.seats.get(candidateUid);
        if (seat && !seat.assigned) {
          seat.assigned = true;
          ch.seatId = candidateUid;
          seatIdx = (seatIdx + i + 1) % roomSeats.length;
          ...
          if (path.length > 0) {
            ...
            // Teleport if no path (different room zone, blocked by walls)
            ch.tileCol = seat.seatCol;
            ch.tileRow = seat.seatRow;
            ch.x = seat.seatCol * TILE_SIZE;
            ch.y = seat.seatRow * TILE_SIZE;
            ch.state = CharacterState.TYPE;
            ch.dir = seat.facingDir;
            ch.frame = 0;
            ch.frameTimer = 0;
      ...
      // Fallback: teleport to room center if no seats available
      if (!assigned) {
